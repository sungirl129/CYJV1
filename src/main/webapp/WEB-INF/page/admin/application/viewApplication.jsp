...
<form action="/admin/processApplication?pageNum=${nowPage.pageNumber}" method="post">
    <c:forEach var="list" items="${nowPage.data}">
        <div class="row">
            <c:forEach var="item" items="${list}">
                <div class="col-xs-6 col-md-3">
                    <div class="thumbnail">
                        <input type="checkbox" name="selectApplication" value="${item.get("applicationModel").id}">
    ...
    <div>
        <button type="submit" name="access" class="btn btn-primary" value="yes">审批通过</button>
        <button type="submit" name="access" class="btn" value="no">不通过</button>
    </div>
...
<div class="foot">
    <p>
        ...
        <a href="/admin/viewApplication?pageNum=1">首页</a>
        <c:choose>
            <c:when test="${requestScope.nowPage.pageNumber > 1}"><a href="/admin/viewApplication?pageNum=${nowPage.pageNumber-1}">上一页</a></c:when>
            <c:otherwise>
                上一页
            </c:otherwise>
        </c:choose>
        <c:choose>
            <c:when test="${nowPage.pageNumber < nowPage.totalPage}"><a href="/admin/viewApplication?pageNum=${nowPage.pageNumber+1}">下一页</a></c:when>
            <c:otherwise>
                下一页
            </c:otherwise>
        </c:choose>
        <a href="/admin/viewApplication?pageNum=${nowPage.totalPage}">尾页</a>
    </p>
</div>
</body>
</html>
